...
export class IntegratedMap implements OnInit {
  ...
  private readonly selectedStop = this.store.selectSignal<BusStop | undefined>( stop );
  private readonly selectedVehicleId = this.store.selectSignal<string | undefined>( selectedVehicleId );
  private readonly preferredStops = this.store.selectSignal<BusStop[]>( preferredStops );
  ...
  ngOnInit(): void {
    ...
    map.on( 'singleclick', ( event: MapBrowserEvent ) => {
      ...
      if ( stopFeature ) {
        ...
        this.store.dispatch( BusesActions.loadBuses( {stop: {stopId: code, description: description}} ) );
      }
    ...
    return [
      ...
      new Style( {
        text: new Text( {
          ...
          textAlign: 'center',
          textBaseline: 'top',
          fill: new Fill( {color: isPreferred ? tertiary : secondary} )
        } )
      } )];
  }
